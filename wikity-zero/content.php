...
if (is_home() or is_search() or is_category()) :
  ...
  /* Clean up for "pinboard" display */
  
  $stripped_content = str_replace("\r\n","\n",$content);
  $stripped_content = str_replace("\n\n","\n",$stripped_content);
  $stripped_content = str_replace("|","",$stripped_content);
  $stripped_content = str_replace("--","",$stripped_content);
  $stripped_content = str_replace("###","<br>",$stripped_content);
  $stripped_content = preg_replace("/!?\[[^\]]*]\([^\)]*\)/","",$stripped_content);
  $stripped_content = trim(wp_strip_all_tags($stripped_content));
  $synopsis = substr($stripped_content,0,370).'...';
...
<input  type="submit" style="font-size:10px" onclick="openEditBox('<?php echo get_the_ID(); ?>', 1)" value="Edit"></div>
<br></div>

  	<div style="display:none;  height:90%" id="entryedit<?php echo get_the_ID(); ?>" >
	    <form method="post" style="height:90%">
		    <input name="title" type="hidden" id="formtitle" value="<?php the_title(); ?>">
 ...
 /* Special Links */
 $content = str_replace('[cite]','[<sup>&deg;</sup>]',$content);
 $content = str_replace('[link]','[<small>(Link)</small>]',$content);
 $content = str_replace('[site]','[<small>(Site)</small>]',$content);
 $content = str_replace('[article]','[<small>(Article)</small>]',$content);
 $content = str_replace('[download]','[<small>(Download)</small>]',$content);
 $content = str_replace('[source]','[<small>(Source)</small>]',$content);
 $content = str_replace("\n&gt;","\n>",$content);
...
/** $content = preg_replace_callback('/\[(https?|ftp):\/\/([^ ]*) ([^ \]]*)]/', 'ext_links', $content); **/
// $content = preg_replace_callback('/\-\-\-\-*/m', 'add_ann', $content);
