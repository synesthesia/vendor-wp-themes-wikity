...
/************* Provider Site (fork-from) ***************

$pagesource = $_GET["source"];

if (strlen($pagesource)){
    $page = get_posts( array( 'name' => $pagesource ) );
    if ( $page )
    {
  ...
  $sql = $wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_name = '$pagesource'");
  $my_id = $wpdb->get_var($sql);
  $sql = $wpdb->prepare("SELECT post_date FROM $wpdb->posts WHERE post_name = '$pagesource'");
  $my_createdate = $wpdb->get_var($sql);
  $history = '';
   ...
   if ($history == ''){
     $update_str ='{
      ...
      "date":'.(strtotime($my_createdate) * 1000).',
      "item": {
        "title": "'.str_replace('"','\"',get_the_title( $my_id )).'"
      }
    }';
   } else {
     $update_str = $history;
   ...
   $update_str .= ',
   {
      ...
      "site": "'.get_site_url().'",
      "date": '.(intval(time()) * 1000).',
      "author": "'.md5(strtolower(trim(get_the_author_meta('user_email',$page[0]->post_author)))).'",
      "name": "'.get_the_author_meta( 'display_name', $page[0]->post_author).'",
      "slug": "'. $page[0]->post_name.'",
      "chars": '. strlen($page[0]->post_content).'
    }';

   echo '<updatehistory>'.$update_str.'</updatehistory>';
    die('');
}
*/
...
<?php if (strlen($_GET['sourceurl']) == 0) :  ?>
  ...
  <article id="post-stub" class="post-1544 post type-post status-publish format-standard hentry category-uncategorized">
        ...
        <article id="post-edit" style="margin-top: 5px; width:655px" class="post-1544 post type-post status-publish format-standard hentry category-uncategorized">
      	...
      	<div class="entry-content" >
      		  <form name="createform" id="createform" method="post">
      		    Title: <input name="title" id="formtitle" style="width:90%" value="<?php echo $form_t; ?>"><br><hr>
      		    <textarea name="content" id="formcontent" style="height:200px; font-size:small"><?php echo $form_c; ?></textarea>
